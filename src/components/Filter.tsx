...
export default function Filter({
    ...
    return (
        <div className={` flex items-center gap-2 ${className}`} {...props}>
            <p
                className={` font-bold text-[14px] text-secondary-900`}
            >
                {title}
            </p>
            <Dropdown
                theme={dropdownTheme}
                label={
                    <p className=" whitespace-nowrap overflow-hidden text-white text-ellipsis max-w-[250px]">
                        {choosen || "All"}
                    </p>
                }
                dismissOnClick={true}
...
const dropdownTheme: CustomFlowbiteTheme["dropdown"] = {
    arrowIcon: "ml-2 h-4 w-4 text-secondary-950 text-white",
    content: "py-1 text-secondary focus:outline-none",
    floating: {
        ...
        item: {
            container: " w-[180px]",
            base: "flex flex-row-reverse items-center justify-between py-2 px-4 text-sm text-secondary cursor-pointer w-full hover:bg-secondary-100 focus:bg-secondary-100 ",
        ...
        target: " max-w-[250px] h-full w-max border-2 border-surface-grey02 text-ellipsis flex bg-surface-grey01 text-secondary-950 bg-secondary transition duration-200 enabled:hover:bg-primary-200 enabled:active:bg-primary-300",
    },
