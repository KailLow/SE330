...
export default function UploadFile({
    ...
    return (
        <div
            className={`flex flex-col w-full h-fit items-end justify-center ${className}`}
        >
            <Label
                htmlFor={`dropzone-file-${index || 0}`}
                className={` px-4 flex w-fit cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed border-secondary-300 bg-secondary-50 hover:bg-secondary-100 ${
                    isCompact ? "h-12" : "h-12"
                    // file || defaultValue ? "h-64" : "h-64"
                }`}
            >
                <div className="flex flex-row gap-4 items-center justify-center pb-6 pt-5">
                    {isCompact ? (
                        <svg
